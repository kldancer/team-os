...
2. 分析与执行分离。角色分四档，全部显式绑定且**可热替换**（档位属角色，换绑不换档）：**分析档** `@plan_owner`（GLM-5.3，订阅）做裁决型规划、状态冻结与最终综合；**研判档** `@plan_alt`（K3-256K 起草）/`@deep_review`（K3 红队，与 owner 异厂）只出草案与 findings；**视觉档** `@ui_deep`（K3，前端审美第一）做设计判断，`@ui_qa`（GLM-5.3-Flash）做截图事实核对；**执行档** `@fast_worker` 是池：GLM-5.3-Flash（订阅优先）→ 免费 Flash → DeepSeek（按量）。强制路由：起草型规划至少一半交 `@plan_alt`；UI 与视觉验收必须经视觉档；冻结候选必须有研判档 findings 或显式豁免。主会话只做决策、派工与收据，不自己跑 bash/read/grep/edit；探索与批量读取一律派执行档。其余工作经 Profile 的 `task.agentModelOverrides`（泛型 `task`/`scout`/`sonic`）落实；`default` 指向分析档 GLM-5.3；订阅告急按 `quotaWindows` runbook 显式重绑（owner 回摆 K3-256K 或 standby GPT），替换一律经 `temporaryBindings` 登记并披露。订阅窗口 ≥70% 时把重评审排非高峰，≥90% 或耗尽时改派另一订阅档并披露；回退链只在同一计费档内。末端车道不要求额外角色绑定。用户可显式覆盖，但模型或 fallback 不得静默改变负责人。路由不是靠记忆：规划后用 `scripts/route_work.py` 生成分档派工清单（`juspctl plan` 输出的 `dispatchHint` 即该命令），收尾用 `juspctl close`，它会先跑 `check_role_routing.py --task <id> --gate`，订阅档直连生产或 UI 未过视觉档会直接阻断 close（只能整改或显式 `--skip-role-routing` 覆盖）。
