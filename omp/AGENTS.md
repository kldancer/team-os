...
2. 分析与执行分离。GPT-5.6 Sol 与 GPT-6 Astra 是保留角色：Sol 只承担结果合同、规划、跨边界设计与状态冻结、集成复核和最终综合；Astra 只承担 UI/前端深度设计与深度评审。其余工作（侦察、批量取证、有界实现、门禁/冒烟/刷新/生产事实执行、收据写入、机械文档与提交准备）由执行档承担：`@fast_worker` 与 Profile 的 `task.agentModelOverrides`（泛型 `task`/`scout`/`sonic`）；`default` 角色指向保留档，供主 Session 和未绑定回退使用，不得让执行工作落在 GPT 档位。提交、推送、机械校准这类末端车道不要求额外角色绑定。用户可显式覆盖，但模型或 fallback 不得静默改变负责人。
...
7. 派工先写自足派工包（仓库绝对路径、写集合、禁止读取、变更步骤、内联合同、验收命令），下游不从设计正文或规划正文反推意图；保留角色不做原始探索，改由快速只读角色取回带锚点的压缩证据包。只有独有证据、互斥写集合或高风险独立验证值得协作；worker 必须在 Agent Hub 可检查和停止，写入由负责人复核真实 diff 与集成入口，复核以收据摘要、diff 统计和定点读取为主，不重读改动全文。
8. 模型和 Provider 通过 `@plan_owner/@ui_deep/@deep_review/@fast_worker` 等运行时角色绑定；首次分派或映射变化后必须在 Agent Hub 核对 resolved model：保留角色不得静默回退，执行角色不得静默升级到 GPT 档位；映射缺失或 fallback 不符时停止 worker。切换、fallback 和实际模型必须显式披露并进入适用收据。不得记录密钥、认证信息或完整 Transcript。
