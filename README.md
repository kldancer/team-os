...
当前已完成 TOS0～TOS4：在最小目录、通用短内核和项目适配器之上，已建立科学协作与软件交付操作模型、自适应拓扑、角色/专业能力目录、模型准入和长期观察合同。用户先在同一个 Michael Session 中讨论；明确说或点击“按结论开始推进”后，Michael 才按需读取项目权威并提交 Plan Manifest，Munder Main Process 只做确定性校验、真实 Agent/PTY/Session 分配和阶段投影。计划只引用项目路径，不复制正文或 Transcript；该动作可授予适配器允许的项目内本地实现与适用验证，远端、生产、破坏性和 Git 写始终关闭。模型自动选择仍不是默认能力，工作流质量、效率和模型组合在长期使用中持续校准。
