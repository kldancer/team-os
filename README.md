...
安装到运行时的只是短内核和按需 Skill，完整 Team OS 文档不会自动塞入每个任务上下文。Codex、Pi、OMP 的入口分别见 [`codex/README.md`](codex/README.md)、[`pi/README.md`](pi/README.md) 和 [`omp/README.md`](omp/README.md)。自然语言需求会按 [`workflows/request-compiler.md`](workflows/request-compiler.md) 自动编译为最小结果合同；UI 设计与前端交付的方法见 [`workflows/ui-design-frontend.md`](workflows/ui-design-frontend.md)，真实浏览器取证见 [`workflows/real-browser-verification.md`](workflows/real-browser-verification.md)；这些方法都不替代项目的设计系统、机器 Gate 或生产授权。派工包、读取纪律与执行模型分档见 [`workflows/context-economy.md`](workflows/context-economy.md)，派工包骨架见 [`templates/worker-pack.md`](templates/worker-pack.md)；模型路由与运行绑定的核对入口是 [`scripts/check_model_routes.py`](scripts/check_model_routes.py)。
