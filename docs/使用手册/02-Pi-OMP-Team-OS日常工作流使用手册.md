...
```text
工作纪律：规划至少一半交 @plan_alt 起草，裁决与综合留分析档；UI 与视觉验收必经视觉档；冻结候选至少一条异厂 findings；批量读取与命令执行交执行档；主线只做决策、派工与收据。默认停在 k3-256k，1M 只在显式说明或越阈值晋升时出现；窗口 ≥90% 或耗尽改派另一订阅档并披露。worker 只回带 文件:行号 的压缩证据包，同一事实只读一次，大输出先落 .work 再摘要。机制兜底：照 dispatchHint 派工、close 过两道门禁、health 报停滞与未验证交付。
```
...
```text
排查这个问题：派 scout 取压缩证据包（文件:行号 + 关键原句），主线只做判断，不要自己整份读文件，也不要为它切到 1M。给出可证伪假设、关键证据、根因和最小修复建议。
```
...
5) 收尾跑 juspctl close 过门禁，被阻断先整改，不要用 --skip-* 绕过。
```

B 强化编排：

```text
按以上结论推进，用 orchestrate 强化编排。边界与完成条件以我给的范围为准；先出一份分档派工清单（视觉档基线、研判档 findings、执行档实现、prod-env preflight）再开工，worker 写入由你复核真实 diff 与集成入口。失败预算 2 轮，验收不得静默降级；收尾跑 juspctl close 过两道门禁。
```
...
```text
提交并推送当前稳定变更：只复核既有变更范围、敏感风险与适用门禁证据，按仓库分别 stage、commit、push，不重新设计、不改无关文件。先确认相关任务已通过 juspctl close 的分档门禁与计划收据门禁，被阻断先整改。
```
