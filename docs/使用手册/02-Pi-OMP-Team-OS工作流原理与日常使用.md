...
这些话术不是固定接力队列，而是把一个结果交给最小必要角色。始终只有一个 `@plan_owner` 负责综合；其它角色只在提供独有证据、互斥写集合或独立验收时启用。模型名不写死，使用角色别名即可。

### 7.1 启动与推进：先形成结果合同，再决定是否协作

覆盖日常讨论、方案收敛和“按结论开始推进”。方案未定时，`@plan_alt` 起草，`@plan_owner` 裁决；方案已定且范围小，则直接由 Owner solo。

```text
目标结果：<用户最终能直接验证的结果>
非目标：<明确不做的事情>
权威：<项目 AGENTS.md / 正式设计 / 机器计划 / 代码路径>
范围：读取 <…>；允许写入 <…>；禁止修改 <…>
验收：<命令、真实入口或可观察证据>
停止：<失败预算、越界条件、需要用户决策的分支>

请先固定结果合同，再判断 solo 是否足够。
只有跨边界、未决依赖、独有证据、互斥写集合或高风险独立验收能覆盖协调成本时才派角色：
需要覆盖型规划时让 @plan_alt 起草，@plan_owner 裁决和冻结；需要原始事实时让 @fast_worker 返回带文件:行号的压缩证据包。
最终由 @plan_owner 返回决策基线、实际变更、验收映射和停止条件；不要为 Gate、重试或 worker 建立新的顶层结果。
```

### 7.2 诊断与独立证据：先取证，再决定是否挑战

覆盖 `diagnose`、接口/架构评审和高风险候选冻结。`@fast_worker` 取有界事实；只有首个假设不足或风险足够高时，才增加 `@deep_review` 独立挑战。

```text
请按可证伪诊断闭环处理：先列出最多三个假设、最小判别事实、非目标和停止条件。
让 @fast_worker 只读指定日志、配置、调用链、diff 或运行收据，返回事实/推断/未知、文件:行号和最小下一步；不要修改实现。
如果阻断根因仍不清楚，或这是冻结候选/高风险设计，再让 @deep_review 独立返回 findings、反例、严重级别和最小修正方向；它不改稿、不替 Owner 冻结。
@plan_owner 综合证据，决定根因、修复方向或是否需要用户决策；没有修复授权时保持只读。
```

### 7.3 实施、UI 与验证：按需接入执行和视觉能力

覆盖 `deliver-change`、UI 交付、`guard`/`smoke` 和收尾。普通小改动由 Owner 直接完成；批量/机械工作交 `@fast_worker`。只有真实页面验收才接入 `@ui_deep`、`@ui_impl`、`@ui_qa`。

```text
按已定结论推进，不重新设计；先确认写集合、验收和失败预算。
小范围低风险修改由 @plan_owner solo；需要批量取证、命令或互斥写集合时，给 @fast_worker 自足派工包（绝对路径、禁止读取、步骤、验收、停止条件），它只按包修改并返回实际路径、VERDICT、收据和风险。
如果存在真实 UI/交互：让 @ui_deep 先冻结视觉基线和截图验收点，再把视觉合同交 @ui_impl 实现，由 @ui_qa 对真实入口做事实核对；纯后端任务不启动视觉角色。
只运行机器计划判定适用的 Gate/healthcheck/smoke，返回 PASS|FAIL|SKIP 和收据；不扩大成全量构建或部署。
最后由 @plan_owner 复核真实 diff、收据和授权外边界，决定是否关闭结果或回同一 Owner 修复。
```
